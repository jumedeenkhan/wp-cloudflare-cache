<?php
/**
 * One-time carry-over of the old plugin's options and Page Rule into the current sfcfc_ plugin.
 *
 * @package SuperFlare
 */
...
/**
 * Migrates legacy plugin options (wpcc_options/wpcc_config, cfca_options/cfca_config) and the
 * old Page Rule into the current sfcfc_ plugin, replacing it with a Cache Rule.
 */
class SFCFC_Migration {

	const NEEDS_ATTENTION_OPTION = 'sfcfc_migration_needs_attention';
	...
	/**
	 * Migrates legacy options once, checking the most recent legacy option name (cfca_)
	 * before the older one (wpcc_).
	 *
	...
	public static function maybe_migrate( $instance ) {
		...
		if ( false !== get_option( 'cfca_options', false ) ) {
			self::migrate_from_cfca( $instance );
			return;
		}

		if ( false !== get_option( 'wpcc_options', false ) ) {
	...
	/**
	 * Carries over cfca_ options and config as-is; only the option names changed.
	 *
	 * @param SFCFC_Cache $instance
	 */
	private static function migrate_from_cfca( $instance ) {
		update_option( 'sfcfc_options', get_option( 'cfca_options', array() ), 'yes' );

		$old_config = get_option( 'cfca_config', array() );

		if ( ! empty( $old_config ) ) {
			$instance->set_config( $old_config );
			$instance->update_config();
		}

		/**
		 * The current hook is scheduled fresh on demand; see SFCFC_Purge::maybe_start_queue_cron().
		 */
	...
	private static function migrate_options( $instance ) {
		...
		$migrated = array(
			'cf_auth_method'     => 'key', // Key auth only.
			...
			'cache_exclude_urls' => '',
			'purge_homepage_on_update'  => ( 'on' === ( $old['purge_homepage'] ?? '' ) ) ? 'on' : '',
			'purge_homepage_on_trash'   => ( 'on' === ( $old['purge_homepage'] ?? '' ) ) ? 'on' : '',
			'purge_on_comment_approved' => ( 'on' === ( $old['purge_on_comment'] ?? '' ) ) ? 'on' : '',
			'purge_on_comment_deleted'  => ( 'on' === ( $old['purge_on_comment'] ?? '' ) ) ? 'on' : '',
			'bypass_sitemap'     => 'on',
			'bypass_robots'      => 'on',
		);

		update_option( 'sfcfc_options', $migrated, 'yes' );
	...
	public static function maybe_render_notice() {
		...
		<div class="notice notice-warning">
			<p>
				<strong><?php esc_html_e( 'SuperFlare Cache — Setup Needed', 'wp-cloudflare-cache' ); ?></strong><br>
				...
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=superflare-settings' ) ); ?>"><?php esc_html_e( 'Go to Settings', 'wp-cloudflare-cache' ); ?></a>
			</p>
		</div>
		<?php
	}
